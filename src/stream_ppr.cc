...
StreamPPR::StreamPPR(np::ndarray x_np, np::ndarray edge_index_np, float alpha, float epsilon):
    ...
    for (int i=0;i<num_node_;i++) check_push(i);

    delta_rv_ = new float[dim_];

    // Output on screen
    std::cerr << "::: Graph(num_node: " << num_node_ << ", num_edge_: " << num_edge_ << ")." << std::endl;
...
void StreamPPR::check_push(int64_t u) {
...
void StreamPPR::basic_propagation() {
    ...
    while (!q.empty()) {
        ...
        for (int i=0;i<(int)edge_[s].size();i++) {
            ...
            if (t == s) continue;
            assert(out_deg_[s] > 0);
            r_[t] += ((1-alpha_) / out_deg_[s]) * r_[s];
            check_push(t);
        }
        cblas_sscal(dim_, 0, r+s*dim_, 1);
        r_[s] *= 0;
    }
